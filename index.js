...
(function (wid, dcm) {
	...
	function MultiPicker(config) {
		...
		this.titleProp = config.titleProp;
		...
		var htmlDom = dcm.getElementsByTagName('HTML')[0]
		...
		this.idxArr    = [];
		this.jsonArr   = [];
		this.liHeight = getStyle(htmlDom, 'fontSize') ? parseInt(getStyle(htmlDom, 'fontSize')) * .7 : 40;
		this.maxHeight = [];
		this.distance  = [];
		this.start     = {
			Y: 0,
	...
	MultiPicker.prototype = {
		...
		initDomFuc: function () {
			...
			html += '<div class="multi-picker-bg" id="multi-picker-bg-' + _this.container + '">'
				...
				+ '<div class="multi-picker-tit">'+ _this.titleProp +'</div>'
				+ '<div class="multi-picker-btn"><img id="picker-btn-cancel" src="data:image/png;base64,"/></div>'
				+ '</div>'
				...
				+ '</div>'
				+ '<div class="picker-confirm-btn"><div class="btn" id="multi-picker-btn-save-' + _this.container + '">确定</div></div>'
				+ '</div></div>';
		...
		initReady: function (idx, target) {
			...
			loop(0, _this.idxArr.length, function (i) {
				...
				var linode = $id('multi-picker-' + _this.container + '-' + i).querySelectorAll('li')
				var selectLi = _this.distance[i] / _this.liHeight + 2
				loop(0, linode.length, function (j) {
					linode[j].style.color = ''
					linode[selectLi].style.color = '#FF663F';
				})
		...
		initBinding: function () {
			...
			on('touchstart', 'picker-btn-cancel', function () {
				bg.classList.remove('multi-picker-bg-up');
				container.classList.remove('multi-picker-container-up');
				setTimeout(function () {
					bg.classList.remove('multi-picker-bg-delay');
